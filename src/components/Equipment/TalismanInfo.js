...
// Component that renders the information of a talisman
const TalismanInfo = () => {
    const [talisman, setTalisman] = useState({})
    const location = useLocation()

    useEffect(() => {
        // Getting the talisman by id
        talismanService
            .get(`talismans/${location.state.id}`)
            .then(res => {
                setTalisman(res)
            })
    }, [location.state.id])
                ...
                <Row>
                    <Col lg={6} md={12} className='d-flex justify-content-center align-self-center'>
                        <div className='img-container d-flex justify-content-center align-items-center'>
                            <img src={talisman.image} alt='img' />
                        </div>
                    </Col>
